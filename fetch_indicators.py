...
<div class="cbox">
  ...
  <div style="display:grid;grid-template-columns:20px 8px 55px repeat(8, 60px) minmax(110px,1fr);gap:18px;padding:0 0 8px;font-size:10px;color:#6b6b7a;border-bottom:1px solid #2a2a35;">
  <span></span><span></span><span style="text-align:left;">종목</span><span style="text-align:center;">현재가</span><span id="sortScoreHeader" style="text-align:center;cursor:pointer;user-select:none;">점수 <span id="sortScoreArrow" style="color:#6b6b7a;">▾</span></span><span style="text-align:center;">업사이드</span><span style="text-align:center;">다이버전스</span><span style="text-align:center;">트리거</span><span style="text-align:center;">RSI</span><span id="sortAddonHeader" style="text-align:center;cursor:pointer;user-select:none;">눌림목점수 <span id="sortAddonArrow" style="color:#3a3a45;">▾</span></span><span style="text-align:center;">신호</span><span></span>
  </div>
  <div id="rankList"></div>
...
(function(){
 ...
 function signalBadge(item){
    ...
    // 우선순위 1: 다이버전스 무효화 (위험 신호)
    if (dv && dv.active) {
      var col1 = '#fb7185', bg1 = 'rgba(244,63,94,0.18)';
      ...
      return '<span style="position:relative;display:inline-block;" class="sig-wrap">'
        + '<span style="height:19px;padding:0 8px;border-radius:999px;background:'+bg1+';border:1px solid '+col1+';color:'+col1+';font-size:10px;font-weight:700;display:inline-flex;align-items:center;cursor:help;white-space:nowrap;">다이버전스 무효화</span>'
  ...
  function renderRow(item, container, isLast){
    ...
    head.style.cssText = 'display:grid;grid-template-columns:20px 8px 55px repeat(8, 60px) minmax(110px,1fr);gap:18px;align-items:center;cursor:pointer;';
    ...
    .then(function(d){
      ...
      function renderAll(){
        var sorted = allRankings.slice().sort(function(a,b){
          var av = a[currentSortKey], bv = b[currentSortKey];
