...
def find_signals(ticker, benchmark_close):
    ...
    for i in range(start, end):
        ...
        # 최근 20일 내 반복된 다이버전스들의 RSI백분위가 상승추세인지 확인
        recent_div_positions = [p for p in div_positions if i - 20 <= p < i]
        repeat_pctile_trend = None
        if len(recent_div_positions) >= 2:
            first_p = recent_div_positions[0]
            last_p = recent_div_positions[-1]
            pctile_first = pctile_series.iloc[first_p] if not pd.isna(pctile_series.iloc[first_p]) else None
            pctile_last = pctile_series.iloc[last_p] if not pd.isna(pctile_series.iloc[last_p]) else None
            if pctile_first is not None and pctile_last is not None:
                repeat_pctile_trend = bool(pctile_last > pctile_first)  # True=반복될수록 백분위 상승
        ...
        results.append({
            ...
            "repeat_count": repeat_count, "repeat_pctile_trend": repeat_pctile_trend,
            "vol_divergence": vol_divergence, "vol_spike": vol_spike,
...
def main():
    print("벤치마크(SPY) 로딩 중...")
    bench = yf.Ticker("SPY").history(period="2y")["Close"]
    ...
    print(f"\n=== 대형주 다이버전스 신호 {len(all_signals)}건, {FORWARD_DAYS}일 뒤 기준 ===\n")
    ...
    for cnt in range(0, 21):
        sub = [s for s in all_signals if s["repeat_count"] == cnt]
        wr, avg = win_rate(sub)
        if wr is not None and len(sub) >= 10:
            print(f"  {cnt}회: 표본 {len(sub)} | 승률 {wr:.1f}% | 기준대비 {wr-baseline_wr:+.1f}%p")
    sub = [s for s in all_signals if s["repeat_count"] >= 21]
    wr, avg = win_rate(sub)
    if wr is not None:
        print(f"  21+회: 표본 {len(sub)} | 승률 {wr:.1f}% | 기준대비 {wr-baseline_wr:+.1f}%p")

    print("\n[반복될수록 RSI백분위 상승/하락 추세]")
    trend_up = [s for s in all_signals if s["repeat_pctile_trend"] is True]
    trend_down = [s for s in all_signals if s["repeat_pctile_trend"] is False]
    for label, sub in [("반복될수록 백분위 상승(개선)", trend_up), ("반복될수록 백분위 하락(악화)", trend_down)]:
        wr, avg = win_rate(sub)
        if wr is not None:
            print(f"  {label}: 표본 {len(sub)} | 승률 {wr:.1f}% | 기준대비 {wr-baseline_wr:+.1f}%p")

    print("\n해석: 반복횟수가 특정 구간에서 정점을 찍는지, 궤적 방향이 결정적인지 확인.")
